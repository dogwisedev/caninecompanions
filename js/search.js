document.addEventListener('DOMContentLoaded', () => {
  ...
  function search() {
    ...
    if (trainers.length === 0) {
      alert('Trainer data is still loading…');
      return;
    }
    ...
    fetch(`https://api.zippopotam.us/us/${query}`)
      ...
      .then(data => {
        ...
        trainers.forEach(t => {
          ...
          // CHANGE: Uses trainer's specific distance from JSON (defaults to 50 if missing)
          const maxAllowed = t.maxDistance || 50;
  ...
  function renderResults(nearby, online) {
    ...
    if (nearby.length > 0) {
      ...
      header.innerHTML = `<div class="heading_container heading_center"><h2>In-Person <span>Trainers</span></h2></div><hr style="max-width: 300px; margin: 30px auto; border-color: #808f71;">`;
    ...
    if (online.length > 0) {
      const header = document.createElement('div');
      header.className = 'col-12 text-center mt-5';
      header.innerHTML = `<div class="heading_container heading_center"><h2>Available for <span>Online Training</span></h2></div><hr style="max-width: 300px; margin: 30px auto; border-color: #808f71;">`;
      results.appendChild(header);
      const row = document.createElement('div');
      row.className = 'row w-100 m-0 justify-content-center';
      results.appendChild(row);
      ...
      <div class="trainer-result-card h-100 d-flex flex-column">
        <div class="img-box">
          <img src="${t.image || ''}" alt="${t.name}" class="trainer-result-img"
               onerror="this.src='images/default-trainer.jpg'">
        </div>
        <div class="trainer-result-info d-flex flex-column flex-grow-1">
          <h3 class="trainer-result-name mb-2">${t.name} ${badge}</h3>
          <p class="trainer-result-region mb-2">${locationLine}</p>
          <p class="trainer-result-price mb-3">${t.price ? `$${t.price}/session` : 'Price on request'}</p>
          <p class="trainer-result-blurb flex-grow-1 mb-3">${t.blurb || 'Certified professional dog trainer.'}</p>
          <div class="mt-auto">
